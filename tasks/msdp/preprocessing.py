...
def get_args():
    ...
    parser.add_argument(
        "--knwl_ref_file", type=str, default=None, help="path of the knowledge reference file"
    )
    parser.add_argument(
        "--resp_ref_file", type=str, default=None, help="path of the knowledge reference file"
    )
    parser.add_argument(
        "--knwl_gen_file", type=str, default=None, help="path of the generated knowledge file"
    )
    ...
    for i, sample in enumerate(tqdm(dialog_data)):
        ...
        for j, turn in enumerate(dialog):
            ...
            if "wizard" in speaker:
                ...
                fproc.write(
                    topic + "\t" + dialog_context + "\t" + knowledge + "\t" + response + "\n"
                )
...
def get_database(test_datapath, train_datapath, data_type):
    ...
    assert data_type in ["wow_seen", "wow_unseen", "woi"], "Please input a correct data type!!"
...
if __name__ == "__main__":
    ...
    if args.func == "process_wow_dataset":
        ...
        prompt_selection_for_knowledge_generation(
            args.test_file, args.train_file, args.model_file, args.processed_file, args.data_type
        )
