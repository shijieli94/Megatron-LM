...
class ORQAEvaluator(object):
    ...
    def generate_query_vectors(self, qa_data, split):
        ...
        for batch in dataloader:
            ...
            query_tokens, query_mask, query_types, query_len, reference = process_nq_batch(batch)
    ...
    def evaluate(self, qa_data, split):
        ...
        if local_rank != 0:
            distance = torch.empty(
                device_count * len(query_tensor), args.faiss_topk_retrievals, dtype=torch.float32
            ).cuda()
            topkindex = torch.empty(
                device_count * len(query_tensor), args.faiss_topk_retrievals, dtype=torch.int64
            ).cuda()
