...
class OptimizerParamScheduler(object):
    ...
    def get_wd(self):
        ...
        if self.wd_incr_style == 'linear':
            ...
            raise Exception(
                '{} weight decay increment style is not supported.'.format(self.wd_incr_style)
            )
    ...
    def get_lr(self):
        ...
        if self.lr_warmup_steps > 0 and self.num_steps <= self.lr_warmup_steps:
            return self.init_lr + (
                (self.max_lr - self.init_lr) * float(self.num_steps) / float(self.lr_warmup_steps)
            )
    ...
    def load_state_dict(self, sd):
        ...
        if 'start_wd' in sd:
            ...
            self.wd_incr_steps = self._check_and_set(
                self.wd_incr_steps, sd['wd_incr_steps'], "total number of weight decay iterations"
            )
