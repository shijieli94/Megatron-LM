...
def _allocate_recv_buffer(batch_size, sequence_length):
    ...
    return torch.empty(
        recv_size, dtype=_get_recv_buffer_dtype(args), device=torch.cuda.current_device()
    )
...
def _no_pipelining_forward_step(
    ...
    output_tensor = _forward_step_helper(
        model, tokens, position_ids, attention_mask, inference_params, recv_buffer=recv_buffer
    )
