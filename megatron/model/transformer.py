...
from megatron.model.utils import attention_mask_func, erf_gelu, get_norm, openai_gelu
...
class ParallelTransformerLayer(MegatronModule):
    ...
    def retro_encoder_cross_attention(
        self, retriever_output, norm_input, norm_output, bias_dropout_add_func
    ):
        ...
        chunked_outputs = norm_output.reshape(
            self.retro_retrieved_length, -1, self.retro_num_neighbors, d
        )
        chunked_outputs_before_norm = norm_input.reshape(
            self.retro_retrieved_length, -1, self.retro_num_neighbors, d
        )  # [r, bs*l, k, d]
        ...
        for k in range(self.retro_num_neighbors):
            ...
            attention_output, attention_bias = self.inter_attention(
                chunked_output, None, encoder_output=retriever_output  # Q (neighbor embedding)
            )  # K, V (hidden act)
    ...
    def retro_decoder_cross_attention(
        ...
        if self.layer_type == LayerType.retro_decoder_with_retriever:
            ...
            if first_ns > 0:
                ...
                first_chunk, rest_chunk = norm_output[:first_ns], norm_output[first_ns:]
        ...
        with torch.enable_grad():
            ...
            norm_input = norm_input.reshape(self.retro_chunk_length, bs, l, d).permute(
                2, 0, 1, 3
            )  # [l, m, bs, d]
            norm_input = norm_input.reshape(self.retro_chunk_length * l, bs, d)
            norm_input = torch.nn.functional.pad(norm_input, (0, 0, 0, 0, pad, 0), 'constant', 0)[
                :ns
            ]  # [ns, b, d]
    ...
    def forward(
        ...
        if self.layer_type == LayerType.encoder:
            ...
            norm_input, norm_output = self.retro_encoder_cross_attention(
                retriever_output, norm_input, norm_output, bias_dropout_add_func
            )
        elif self.layer_type in (LayerType.retro_decoder, LayerType.retro_decoder_with_retriever):
            retriever_output, norm_input, norm_output = self.retro_decoder_cross_attention(
                retriever_input,
                retriever_output,
                retriever_attn_mask,
                norm_input,
...
class ParallelTransformer(MegatronModule):
    ...
    def __init__(
        ...
        if model_type == ModelType.retro_decoder:
            ...
            self.retro_layer_numbers = np.arange(retro_layer_start, args.num_layers + 1, 3).tolist()
        if model_type == ModelType.retro_encoder:
            self.retro_layer_numbers = [1]

        # Transformer layers.
        if args.retro_add_retriever:
            assert self.recompute_granularity != 'full', "Full recompute not supported for Retro."
            assert (
            ...
            if model_type == ModelType.retro_encoder:
                for layer in self.layers:
                    if layer.self_attention.use_flash_attn:
                        layer.self_attention.core_attention_flash.dropout_p = torch.nn.Dropout(
