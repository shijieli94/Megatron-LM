...
from tasks.vision.segmentation.metrics import PSNR, SSIM
...
def model_provider(pre_process=True, post_process=True):
    ...
    if args.vision_backbone_type == 'vit':
        model = VitInpaintingModel(
            config=config, pre_process=pre_process, post_process=post_process
        )
    elif args.vision_backbone_type == 'mit':
        model = MitInpaintingModel(
            config=config, pre_process=pre_process, post_process=post_process
        )
    else:
...
def loss_func(images, masks, masked_images, outputs, non_loss_data=False):
    ...
    if not non_loss_data:
        ...
        loss = F.mse_loss(flip_masked_outputs, flip_masked_images.float(), reduction="sum")
        ...
        return loss, {"loss": averaged_loss[0], "psnr": averaged_loss[1], 'ssim': averaged_loss[2]}
    else:
