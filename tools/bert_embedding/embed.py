...
from megatron import core, get_args, get_tokenizer, print_rank_0
...
def get_batch(data_iterator):
    ...
    keys = ['text', 'types', 'labels', 'is_random', 'loss_mask', 'padding_mask', 'seq_length']
    ...
    return tokens, types, sentence_order, loss_mask, lm_labels, padding_mask, seq_lengths


def loss_func(loss_mask, sentence_order, seq_lengths, output_tensor, non_loss_data):
...
def forward_step(data_iterator, model):
    ...
    # Get the batch.
    tokens, types, sentence_order, loss_mask, lm_labels, padding_mask, seq_lengths = get_batch(
        data_iterator
    )
    ...
    return output_tensor, partial(loss_func, loss_mask, sentence_order, seq_lengths)
...
class DiskDataParallelBertEmbedder:
    ...
    def __init__(self, batch_size, max_bert_seq_length, block_size, embedder_type):
        self.embedder = BertEmbedder(batch_size, max_bert_seq_length, embedder_type)
        self.block_size = block_size

    def embed_text_blocks(self, name, workdir, text_dataset, missing_embedding_blocks):
    ...
    def embed_text_dataset(self, name, workdir, text_dataset):
        ...
        n_missing_world, missing_embedding_blocks = get_missing_blocks_by_rank(
            workdir, len(text_dataset), self.block_size, validate=validate
        )
        ...
        # Embed batches.
        self.embed_text_blocks(name, workdir, text_dataset, missing_embedding_blocks)
