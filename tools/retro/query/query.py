...
from tools.retro.db.utils import get_merged_train_dataset as get_db_merged_train_dataset
...
def get_index(ondisk=False):
    ...
    # Search parameters.
    faiss.ParameterSpace().set_index_parameter(index, "efSearch", args.retro_query_ef_search)
    faiss.ParameterSpace().set_index_parameter(index, "nprobe", args.retro_query_nprobe)
...
def query_embeddings(
    db_dataset, index, embeddings, chunk_id_range, sample_map, n_chunks_per_sample, verbose=True
):
    ...
    _, query_neighbor_ids = index.search(embeddings, args.retro_query_num_neighbors_query)
    if verbose:
    ...
    for chunk_id in range(min_chunk_id, max_chunk_id):
        ...
        filtered_row += [-1] * (args.retro_query_num_neighbors_save - len(filtered_row))
...
def query_embedding_block(
    db_dataset, index, embeddings, chunk_id_range, sample_map, n_chunks_per_sample
):
...
def query_block_neighbors(db_dataset, query_dataset, index, embedder, block):
    ...
    _, filtered_neighbor_ids = query_embedding_block(
        db_dataset, index, embeddings, block["range"], sample_map, n_chunks_per_sample
    )
...
def query_dataset_neighbors(db_dataset, query_dataset, prefix, neighbor_dir, index, embedder):
    ...
    def validate(f):
        assert (
            f["neighbors"].shape[1] == args.retro_query_num_neighbors_save
    ...
    for block_index, block in enumerate(missing_neighbor_blocks):
        ...
        if block is not None:
            ...
            # Progress.
            print_rank_0(
                "query '%s' block %d / %d ... %s ... mem %.3f gb, %.1f%%."
                % (
                    prefix,
                    block_index,
            ...
            query_block_neighbors(db_dataset, query_dataset, index, embedder, block)
...
def query_pretraining_neighbors():
    ...
    for prefix, info in query_dataset_map.items():
        print_rank_0(" > query '%s' dataset ... %d samples." % (prefix, len(info["data"])))
        query_dataset_neighbors(
            db_dataset, info["data"], prefix, info["neighbor_dir"], index, embedder
        )
