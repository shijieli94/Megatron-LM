...
def init_indexed_dataset_infos():
    ...
    assert len(args.data_path) % 2 == 0, "currently, only blendable dataset is supported."
...
def build_partial_db(
    ...
    for doc_id in pbar:
        ...
        chunk_end_idxs = [min(doc_len, s + args.retro_gpt_chunk_length) for s in chunk_start_idxs]
...
def update_chunk_counts(indexed_dataset_infos):
    ...
    for ds_index, ds_info in enumerate(indexed_dataset_infos):
        ...
        for db_path in tqdm(
            db_paths, "%d/%d, %s" % (ds_index, len(indexed_dataset_infos), ds_info["name"])
        ):
        ...
        ds_info["n_chunks_sampled"] = int(
            args.retro_index_ntrain * ds_info["ratio"] / data_ratio_sum
        )
    ...
    if not os.path.exists(db_path):
        ...
        for ds_idx, ds_info in enumerate(indexed_dataset_infos):
            ...
            individual_doc_offsets = (
                None if n_docs_key is None else get_individual_doc_offsets(ds_idx, ds_info)
            )
